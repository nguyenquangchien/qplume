...
tmpList = [ ]  # initialised here, to be used in `search_DFS`: global scope is necessary for recursion
...
if scenario=='TEST':
    ...
    Ufric = 0 # 0.04 * 0.1 * Wo  # will cause the plume to deflect
    dens_deficit = 0
    eps = 0.005 / (UNIT_LEN**2)
    nuh = 0.002 / (UNIT_LEN**2)
    nuv = 0.002 / (UNIT_LEN**2)
...
width = xRport - xLport  # width of the outfall
...
for cell in meshU.leafList:
    ...
    st = str(int(round(100 * vel / Wo)))
    if st != "0":
        meshU.textList.append([cell['xC'], cell['yC'], st])
